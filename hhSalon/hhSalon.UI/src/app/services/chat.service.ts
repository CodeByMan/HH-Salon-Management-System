...
export class ChatService {
  ...
  public addUser(userId: string){ 
   ...
   public saveMessage(message: Message){
    return this.http.post<Message[]>(`${environment.apiUrl}/${this.url}/save-message`, {
      toId: message.toId,
   ...
   public updateMessage(message: Message){
    return this.http.put(`${environment.apiUrl}/${this.url}`, { id: message.id });
   }
   ...
   createChatConnection(userId: string){   
      ...
      this.chatConnection = new HubConnectionBuilder()
        .withUrl(`${this.chatUrl}/hubs/chat`, { withCredentials: true }).withAutomaticReconnect().build();
          ...
          if(this.router.url != '/chat/' + newMessage.fromId){
              ...
              toastr.info(mess.toString(), 'New message!');
                    ...
                    this.messages.map(
                      ...
                      this.chatConnection?.invoke('ReadPrivateMessage', { id: newMessage.id }).then( () => {  
  ...
  async sendMessage(from: any, to: any, content: string){
        ...
        this.chatConnection?.invoke('ReceivePrivateMessage', message).then( () => {
            ...
            .catch(error => {
              console.log(error);
              toastr.error('The message could not be sent.', 'Chat error');
            }
          )
       ...
       updateMessages(){
          ...
          if(mess != undefined){
              ...
              this.chatList.filter(l => l.userId === mess?.fromId && l.toUserId === this.userId)[0].messageUnreadCount = 0;
                ...
                this.chatConnection?.invoke('ReadPrivateMessage', { id: mess.id }).then( () => {
